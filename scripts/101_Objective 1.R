...
#name: 101_Objective1.R
#date updated: 4/28/2026

#description: objective 1 data prep and analysis, single-season occupancy models 
## comparing point count and ARU data
...
#please excuse poor coding skills, i was a noob :( 
...
rm(list = ls()[!str_detect(ls(), pattern = "enc_")])
...
# LEAST BITTERN
umf_lebi_pc <- unmarkedFrameOccu(y = as.matrix(enc_hist_pc$LEBI),
                              siteCovs = NULL,
                              obsCovs = NULL)

occu.m1_lebi_pc <- occu(~1 ~ 1, data = umf_lebi_pc)
summary(occu.m1_lebi_pc)
confint(occu.m1_lebi_pc, type = "state", level = 0.85)
confint(occu.m1_lebi_pc, type = "det", level = 0.85)
bt <- backTransform(occu.m1_lebi_pc, type = "state")
SE(bt) / coef(bt) * 100 #CV
ci <- confint(bt)
ci[2] - ci[1]



umf_lebi_aru <- unmarkedFrameOccu(y = as.matrix(enc_hist_aru$`Least Bittern`),
                                 siteCovs = NULL,
                                 obsCovs = NULL)

occu.m1_lebi_aru <- occu(~1 ~ 1, data = umf_lebi_aru)
summary(occu.m1_lebi_aru)
confint(occu.m1_lebi_aru, type = "state", level = 0.85)
confint(occu.m1_lebi_aru, type = "det", level = 0.85)
bt <- backTransform(occu.m1_lebi_aru, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]


umf_lebi_comb <- unmarkedFrameOccu(y = as.matrix(enc_hist_comb$`Least Bittern`),
                                  siteCovs = NULL,
                                  obsCovs = NULL)

occu.m1_lebi_comb <- occu(~1 ~ 1, data = umf_lebi_comb)
summary(occu.m1_lebi_comb)
confint(occu.m1_lebi_comb, type = "state", level = 0.85)
confint(occu.m1_lebi_comb, type = "det", level = 0.85)
bt <- backTransform(occu.m1_lebi_comb, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]
...
# AMCO
umf_amco_pc <- unmarkedFrameOccu(y = as.matrix(enc_hist_pc$AMCO),
                                 siteCovs = NULL,
                                 obsCovs = NULL)

occu.m1_amco_pc <- occu(~1 ~ 1, data = umf_amco_pc)
summary(occu.m1_amco_pc)
confint(occu.m1_amco_pc, type = "state", level = 0.85)
confint(occu.m1_amco_pc, type = "det", level = 0.85)
bt <- backTransform(occu.m1_amco_pc, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]
...
umf_amco_aru <- unmarkedFrameOccu(y = as.matrix(enc_hist_aru$`American Coot`),
                                  siteCovs = NULL,
                                  obsCovs = NULL)

occu.m1_amco_aru <- occu(~1 ~ 1, data = umf_amco_aru)
summary(occu.m1_amco_aru)
confint(occu.m1_amco_aru, type = "state", level = 0.85)
confint(occu.m1_amco_aru, type = "det", level = 0.85)
bt <- backTransform(occu.m1_amco_aru, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]


umf_amco_comb <- unmarkedFrameOccu(y = as.matrix(enc_hist_comb$`American Coot`),
                                   siteCovs = NULL,
                                   obsCovs = NULL)

occu.m1_amco_comb <- occu(~1 ~ 1, data = umf_amco_comb)
summary(occu.m1_amco_comb)
confint(occu.m1_amco_comb, type = "state", level = 0.85)
confint(occu.m1_amco_comb, type = "det", level = 0.85)
bt <- backTransform(occu.m1_amco_comb, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]
...
# PBGR
umf_pbgr_pc <- unmarkedFrameOccu(y = as.matrix(enc_hist_pc$PBGR),
                                 siteCovs = NULL,
                                 obsCovs = NULL)

occu.m1_pbgr_pc <- occu(~1 ~ 1, data = umf_pbgr_pc)
summary(occu.m1_pbgr_pc)
confint(occu.m1_pbgr_pc, type = "state", level = 0.85)
confint(occu.m1_pbgr_pc, type = "det", level = 0.85)

bt <- backTransform(occu.m1_pbgr_pc, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]


umf_pbgr_aru <- unmarkedFrameOccu(y = as.matrix(enc_hist_aru$`Pied-billed Grebe`),
                                  siteCovs = NULL,
                                  obsCovs = NULL)

occu.m1_pbgr_aru <- occu(~1 ~ 1, data = umf_pbgr_aru)
summary(occu.m1_pbgr_aru)
confint(occu.m1_pbgr_aru, type = "state", level = 0.85)
confint(occu.m1_pbgr_aru, type = "det", level = 0.85)
bt <- backTransform(occu.m1_pbgr_aru, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]


umf_pbgr_comb <- unmarkedFrameOccu(y = as.matrix(enc_hist_comb$`Pied-billed Grebe`),
                                   siteCovs = NULL,
                                   obsCovs = NULL)

occu.m1_pbgr_comb <- occu(~1 ~ 1, data = umf_pbgr_comb)
summary(occu.m1_pbgr_comb)
confint(occu.m1_pbgr_comb, type = "state", level = 0.85)
confint(occu.m1_pbgr_comb, type = "det", level = 0.85)
bt <- backTransform(occu.m1_pbgr_comb, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]
...
# SORA
umf_sora_pc <- unmarkedFrameOccu(y = as.matrix(enc_hist_pc$SORA),
                                 siteCovs = NULL,
                                 obsCovs = NULL)

occu.m1_sora_pc <- occu(~1 ~ 1, data = umf_sora_pc)
summary(occu.m1_sora_pc)
confint(occu.m1_sora_pc, type = "state", level = 0.85)
confint(occu.m1_sora_pc, type = "det", level = 0.85)

bt <- backTransform(occu.m1_sora_pc, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]


umf_sora_aru <- unmarkedFrameOccu(y = as.matrix(enc_hist_aru$Sora),
                                  siteCovs = NULL,
                                  obsCovs = NULL)

occu.m1_sora_aru <- occu(~1 ~ 1, data = umf_sora_aru)
summary(occu.m1_sora_aru)
confint(occu.m1_sora_aru, type = "state", level = 0.85)
confint(occu.m1_sora_aru, type = "det", level = 0.85)
bt <- backTransform(occu.m1_sora_aru, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]


umf_sora_comb <- unmarkedFrameOccu(y = as.matrix(enc_hist_comb$Sora),
                                   siteCovs = NULL,
                                   obsCovs = NULL)

occu.m1_sora_comb <- occu(~1 ~ 1, data = umf_sora_comb)
summary(occu.m1_sora_comb)
confint(occu.m1_sora_comb, type = "state", level = 0.85)
confint(occu.m1_sora_comb, type = "det", level = 0.85)
bt <- backTransform(occu.m1_sora_comb, type = "state")
SE(bt) / coef(bt) * 100
ci <- confint(bt)
ci[2] - ci[1]
