################################################################################L
# 
## file name: 202_Objective2_DataPrep.R
##
## Updated 04/29/2026
## Description: Code to clean up PC and ARU data for objective 2 of TN Marshbirds Paper
################################################################################L
...
site_covs_mean <- data.frame(
  emergent_veg  = rowMeans(yearlySiteCovs$emergent_veg, na.rm = TRUE),
  floating_veg  = rowMeans(yearlySiteCovs$floating_veg, na.rm = TRUE),
  openwater_veg = rowMeans(yearlySiteCovs$openwater_veg, na.rm = TRUE),
  woody_veg     = rowMeans(yearlySiteCovs$woody_veg, na.rm = TRUE),
  water_depth   = rowMeans(yearlySiteCovs$water_depth, na.rm = TRUE)
)
#correlation matrix
cor_matrix <- (cor(site_covs_mean, use = "complete.obs"))
cor_matrix #water depth and woody_veg high

#obs covs
obs_flat <- data.frame(
  precip = as.vector(as.matrix(obsCov$precip)),
  wind   = as.vector(as.matrix(obsCov$wind)),
  tmax   = as.vector(as.matrix(obsCov$tmax))
)
obs_flat <- na.omit(obs_flat)
cor(obs_flat, use = "complete.obs") #wind and tmax

#site covs
site_df <- as.data.frame(sitecovs)
site_df <- site_df[, sapply(site_df, function(x) var(x, na.rm = TRUE) > 0)]
cor(site_df, use = "complete.obs") #woody 500 and palus 500, woody 224 and palus 224